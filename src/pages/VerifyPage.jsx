...
export default function VerifyPage() {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12 sm:px-6 lg:px-8">
      ...
      <h1 className="font-serif text-4xl font-bold">Get verified</h1>
      <p className="mt-2 text-muted-foreground">
        Verification curbs fake accounts and pump campaigns. Approved investors appear on the{' '}
        <Link to="/investors" className="text-primary">verified investors</Link> wall.
      </p>

      {!user ? (
        <Card className="mt-6"><CardContent className="flex flex-col items-center gap-3 py-10 text-center">
          <p className="text-muted-foreground">Sign in to request verification.</p>
        ...
        (() => { const st = STATUS[verif.status]; const Icon = st.icon; return (
          <Card className="mt-6"><CardContent className="py-8 text-center">
            ...
            <p className="mt-2 text-muted-foreground">
              {verif.status === 'approved' ? 'You are listed on the verified investors wall.'
                : verif.status === 'pending' ? 'Your request is under review.'
                : 'Not approved this time.'}
            </p>
            {verif.status === 'rejected' && <Button variant="accent" className="mt-4" onClick={() => setVerif(null)}>Re-apply</Button>}
          </CardContent></Card>
        ); })()
      ) : (
        <Card className="mt-6"><CardContent>
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="How you'll appear" className="mt-1 w-full" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground">Public handle (X / LinkedIn)</label>
              <input value={handle} onChange={(e) => setHandle(e.target.value)} placeholder="@handle" className="mt-1 w-full" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground">Short statement</label>
              <textarea value={statement} onChange={(e) => setStatement(e.target.value)} rows={3} placeholder="A line about your investing focus…" className="mt-1 w-full" />
            </div>
