...
export default function MembershipPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      if (!fullName.trim()) throw new Error(t('Please enter your full name.'));
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12 sm:px-6 lg:px-8">
      <Badge variant="primary" className="mb-3"><Crown className="h-3.5 w-3.5" /> {t('Membership')}</Badge>
      <h1 className="font-serif text-4xl font-bold">{t('Request access')}</h1>
      <p className="mt-2 text-muted-foreground">
        {t('The full preview is opened to approved members. Tell us who you are — we keep the community real.')}
      </p>

      {!user ? (
        <Card className="mt-6"><CardContent className="flex flex-col items-center gap-3 py-10 text-center">
          <p className="text-muted-foreground">{t('Sign in first, then request access.')}</p>
        ...
        (() => {
          ...
          return (
            <Card className="mt-6"><CardContent className="py-8 text-center">
              ...
              <p className="mt-2 text-muted-foreground">{t(st.note)}</p>
              {member.status === 'rejected' && (
                <Button variant="accent" className="mt-4" onClick={() => setMember(null)}>{t('Re-apply')}</Button>
              )}
            </CardContent></Card>
          );
        })()
        ...
        <Card className="mt-6"><CardContent>
          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-muted-foreground">{t('Full name')} *</label>
              <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder={t('Your name')} className="mt-1 w-full" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground">{t('LinkedIn (optional)')}</label>
            ...
            <Button type="submit" variant="accent" className="w-full" disabled={busy}>
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : t('Request access')}
            </Button>
          </form>
        </CardContent></Card>
      )}
