...
export default function StrategiesPage() {
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
      <Badge variant="primary" className="mb-3">{t('Rules-based · monthly rebalance')}</Badge>
      <h1 className="font-serif text-4xl font-bold">{t('Model Strategies')}</h1>
      <p className="mt-2 max-w-2xl text-muted-foreground">
        {t('Transparent, rules-based portfolios built from the Equity Star engine. Each shows its rule, holdings and return since inception versus TASI.')}
      </p>
              ...
              <Card className="h-full transition-shadow hover:shadow-md">
                <CardContent>
                  <div className="flex items-center justify-between">
                    <Badge variant="muted" className={THEME_COLOR[st.theme]}>{t(st.theme)}</Badge>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-border pt-3 text-xs">
                    ...
                    <span className={cn('inline-flex items-center gap-1 font-semibold', alpha >= 0 ? 'text-success' : 'text-destructive')}>
                      <TrendingUp className="h-3.5 w-3.5" /> {alpha >= 0 ? '+' : ''}{alpha.toFixed(1)}% {t('alpha')}
                    </span>
                  </div>
                  <div className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-primary">
                    {t('View strategy')} <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </CardContent>
              </Card>
            </Link>
      ...
      <p className="mt-6 text-xs text-muted-foreground">
        {t('Illustrative back-tested returns assume monthly equal-weight rebalancing and exclude costs. Past performance is not indicative of future results. Not investment advice.')}
      </p>
    </div>
  );
}
