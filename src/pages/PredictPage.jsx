...
export default function PredictPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (ptype === 'price_target' && !(value > 0)) throw new Error('Enter a valid target price');
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <h1 className="font-serif text-4xl font-bold">Make a call</h1>
      <p className="mt-3 max-w-2xl text-muted-foreground">
        Lock in a prediction with a server-side timestamp. It can't be back-dated or edited — when the horizon
        passes, it's scored against the realised price and feeds your track record on the leaderboard.
      </p>
      ...
      <Card className="mt-6 border-primary/30 bg-primary/5">
        <CardContent className="flex flex-wrap items-center gap-3">
          ...
          <span className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1 text-xs text-muted-foreground">
            <Lock className="h-3.5 w-3.5" /> Server-locked
          </span>
        </CardContent>
      </Card>
      ...
      {!user ? (
        <Card className="mt-6"><CardContent className="flex flex-col items-center gap-3 py-10 text-center">
          <p className="text-muted-foreground">Sign in to submit predictions and build your track record.</p>
          <Button variant="accent" onClick={openAuth}>Sign in to play</Button>
        </CardContent></Card>
      ) : (
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          {/* form */}
          <Card><CardContent>
            <h2 className="font-serif text-xl font-bold">New prediction</h2>
            <form onSubmit={submit} className="mt-4 space-y-4">
              ...
              <div className="flex rounded-full border border-border bg-card p-0.5">
                {[['price_target', 'Price target'], ['direction', 'Direction']].map(([v, l]) => (
              ...
              {ptype === 'price_target' ? (
                <div>
                  <label className="text-xs font-semibold text-muted-foreground">Target price (SAR)</label>
                ...
                <div className="flex gap-2">
                  {[['up', 'Up', TrendingUp], ['down', 'Down', TrendingDown]].map(([v, l, Icon]) => (
              ...
              <div>
                <label className="text-xs font-semibold text-muted-foreground">Horizon</label>
                <div className="mt-1 flex gap-2">
                  {[[30, '1 month'], [90, '3 months'], [180, '6 months']].map(([v, l]) => (
              ...
              <Button type="submit" variant="accent" className="w-full" disabled={busy}>
                {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Lock className="h-4 w-4" /> Lock prediction</>}
              </Button>
            </form>
          </CardContent></Card>
          ...
          <Card><CardContent>
            ...
            {mine.length === 0 ? (
              <p className="mt-4 text-sm text-muted-foreground">No predictions yet — lock your first call.</p>
            ) : (
                  ...
                  <div key={p.id} className="flex items-center justify-between rounded-xl border border-border px-3 py-2.5">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {p.prediction_type === 'price_target' ? `Target ${money(p.predicted_value, 'SAR')}` : (p.predicted_value ? 'Direction: Up' : 'Direction: Down')}
                        {' · '}{p.horizon_days}d
                      </div>
                    </div>
                    <Badge variant={p.resolved ? 'success' : 'muted'}>
                      {p.resolved ? `${p.accuracy_pct ?? '—'}%` : <><Lock className="h-3 w-3" /> Locked</>}
                    </Badge>
                  </div>
                ))}
              </div>
