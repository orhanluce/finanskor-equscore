...
function Picker({ onPick, exclude }) {
  ...
  const results = useMemo(() => {
    ...
    return STOCKS
      .filter((st) => !exclude.includes(st.ticker))
      .filter((st) => !s || (st.ticker + ' ' + st.name).toLowerCase().includes(s))
  ...
  return (
    <div className="rounded-2xl border border-dashed border-border bg-card/50 p-4">
      <div className="relative">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('Add a stock…')}
      ...
      <div className="mt-2 space-y-1">
        ...
        {results.length === 0 && <div className="px-3 py-2 text-sm text-muted-foreground">{t('No more matches.')}</div>}
      </div>
    </div>
  );
}
...
const ROWS = [
  { label: 'Equity Star', fn: (s) => `${s.total}/42`, color: (s) => scoreColor(s.total) },
  { label: 'Price', fn: (s) => money(s.price, s.currency) },
  { label: 'Today', fn: (s) => pct(s.change), color: (s) => (s.change >= 0 ? 'text-success' : 'text-destructive') },
  { label: 'Fair value', fn: (s) => money(s.fairValue, s.currency) },
  { label: 'Discount', fn: (s) => `${s.discount >= 0 ? '' : '+'}${Math.abs(s.discount).toFixed(1)}%`, color: (s) => (s.discount >= 0 ? 'text-success' : 'text-destructive') },
  { label: 'P/E', fn: (s) => (s.pe ? s.pe.toFixed(1) : '—') },
  { label: 'P/B', fn: (s) => (s.pb ? s.pb.toFixed(1) : '—') },
  { label: 'Dividend yield', fn: (s) => (s.divYield ? `${s.divYield}%` : '—') },
...
export default function ComparePage() {
  ...
  const stocks = tickers.map((tk) => STOCKS.find((s) => s.ticker === tk)).filter(Boolean);
  ...
  const add = (tk) => setTickers((prev) => (prev.length >= 4 || prev.includes(tk) ? prev : [...prev, tk]));
  const remove = (tk) => setTickers((prev) => prev.filter((x) => x !== tk));

  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <h1 className="font-serif text-4xl font-bold">{t('Compare')}</h1>
      <p className="mt-2 max-w-2xl text-muted-foreground">
        {t('Put up to four names side by side — Equity Star dimensions, valuation, and Sharia status at a glance.')}
      </p>

      <div className="mt-8 grid gap-5 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardContent>
            <h2 className="font-serif text-xl font-bold">{t('Equity Star overlay')}</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">{t('Metric')}</th>
                {stocks.map((s, i) => (
            ...
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.label} className="border-b border-border last:border-0">
                  <td className="px-4 py-3 text-muted-foreground">{t(row.label)}</td>
                  {stocks.map((s) => (
      ...
      {stocks.length === 0 && <div className="mt-10 text-center text-muted-foreground">{t('Add a stock to start comparing.')}</div>}
    </div>
  );
}
