    ...
    <Card className="border-teal/25 bg-teal/5">
      <CardContent>
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-serif text-xl font-bold">Dubai Real-Estate Pulse</h2>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          <Stat value={`${(DLD.transactions / 1000).toFixed(1)}K`} label="Transactions" sub={`+${DLD.transactionsYoY}% YoY`} accent="text-teal" />
          <Stat value={`${DLD.valueAED}B`} label="AED value" sub={`+${DLD.valueYoY}% YoY`} accent="text-success" />
          <Stat value={`${(DLD.investors / 1000).toFixed(1)}K`} label="Investors" sub={`+${DLD.investorsYoY}% YoY`} accent="text-teal" />
        </div>

        <div className="mt-4 flex items-center gap-2 rounded-xl bg-success/10 px-3 py-2 text-sm text-success">
          <TrendingUp className="h-4 w-4" />
          <span><JargonTip term="Momentum" inline>Property momentum</JargonTip> is positive — a tailwind for DFM developers this quarter.</span>
        </div>

        {!compact && reStocks.length > 0 && (
          <div className="mt-4">
            <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Real-estate names</div>
        ...
        <p className="mt-3 text-[11px] text-muted-foreground">Source: Dubai Land Department open data (dubaipulse.gov.ae). Illustrative snapshot.</p>
      </CardContent>
    </Card>
  );
}
