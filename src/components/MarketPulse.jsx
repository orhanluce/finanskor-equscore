    ...
    <Card>
      <CardContent>
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-serif text-lg font-bold">{t('Market Pulse')}</h3>
        </div>

        {/* breadth bar */}
        <div className="mt-4">
          <div className="flex justify-between text-xs text-muted-foreground">
            <span>{up} {t('advancing')}</span><span>{down} {t('declining')}</span>
          </div>
          ...
          <div className="mt-1 text-xs text-muted-foreground">{breadth}% {t('of names higher today')}</div>
        </div>

        <div className="mt-4 grid grid-cols-2 gap-3">
          <Link to={`/stock/${topGainer.ticker}`} className="rounded-xl border border-border p-3 hover:border-success/40">
            <div className="flex items-center gap-1 text-xs text-muted-foreground"><ArrowUpRight className="h-3.5 w-3.5 text-success" /> {t('Top gainer')}</div>
          ...
          <Link to={`/stock/${topLoser.ticker}`} className="rounded-xl border border-border p-3 hover:border-destructive/40">
            <div className="flex items-center gap-1 text-xs text-muted-foreground"><ArrowDownRight className="h-3.5 w-3.5 text-destructive" /> {t('Top loser')}</div>
        ...
        <div className="mt-3 rounded-xl bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
          {t('Average discount to fair value:')}{' '}
          <span className={cn('font-semibold', avgDiscount >= 0 ? 'text-success' : 'text-destructive')}>
            {avgDiscount >= 0 ? '' : '+'}{Math.abs(avgDiscount).toFixed(1)}%
          </span>{' '}— {avgDiscount >= 0 ? t('the board screens cheap on average.') : t('the board screens rich on average.')}
        </div>
      </CardContent>
    </Card>
  );
