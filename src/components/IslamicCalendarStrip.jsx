  ...
  return (
    <div className={cn('border-b text-sm',
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-1.5 sm:px-6 lg:px-8">
        ...
        {sig.note && <span className="hidden truncate text-xs text-muted-foreground md:inline">— {t(sig.note)}</span>}
        {sig.level === 'normal'
          ? null
          : <span className="ml-auto shrink-0 text-[11px] uppercase tracking-wide text-muted-foreground">{t('Hijri risk overlay')}</span>}
      </div>
    </div>
  );
}
